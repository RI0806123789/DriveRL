...
- **打ち切り（`truncated`）の価値目標に、再スポーン後の状態の価値を使わない。**
  `env.step()` は同じステップで respawn するので、`result.obs` は別のエピソードの初期観測
- **勾配は `clip_grad_norm_` の前に測る。** 後だと上限（0.5）に張り付いた値しか見えず、
  画面から**暴れていることが分からない**
...
- **フレーム間の補間は受信した時刻ではなく `simTime` で進める**（`store/playout.ts`）。
  受信の間隔は Windows では 25〜94ms 揺れるので、受信時刻で進めると表示が止まったり跳んだりして、
