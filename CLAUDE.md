...
実用モードの経路追従（`env._autopilot`）が出す操作をそのまま教師にして、
方策の平均を回帰します（`rl/warmstart.py`。行動クローニング）。
実測: 24,000 件・8 エポックで損失 0.172 → 0.003、**60 秒での移動が 0〜3m から
53〜713m** になり、止まったままの台数は 5/8 → 0/8、アクセル指令の平均は -0.82 → +0.75。

- **価値関数と `log_std` には触らないこと。** 経路追従の走りから分かるのは
  「どう操作するか」だけで、「その状態の価値」は出てきません（PPO が学び直します）
