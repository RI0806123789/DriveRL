...
```jsonc
{
  ...
  "vehicles": [
    {
      ...
      "braking": false,       // 制動指令が出ているか（ブレーキランプ）
      "throttle": 0.42,       // 加速指令 -1..1（ペダルの踏み込み）
  ...
  "pedestrians": [                // 街を歩く NPC 歩行者。0 人のときは省略される
    { "id": 0, "x": 12.5, "y": -3.2, "heading": 1.57, "stride": 2.41, "crossing": false }
  ],
...
番号で、その人が消えるまで変わらない。`stride` は手足の振りの位相 [rad] で、**描画の
ためだけに送る**（歩く速さから積分すると、フレームを落としたときに位相がずれる）。
`crossing` は車道を横断中かどうか。

- 歩行者は**道路網の歩道**（車道の端から
  `PEDESTRIAN_SIDEWALK_MARGIN` = 1.7m）を歩き、交差点で車道を横断する。
- **どの車からも 220m 以上離れた歩行者は、見えないところで車の近くへ回される**
  （`x` / `y` が跳ぶ）。広いマップで 1 人も画に写らなくなるのを防ぐためで、金沢
  （12.3km 四方）に 64 人を一様に撒くと 0.42 人/km² にしかならず、実測で真値 0 件だった。
  **クライアントは車両と同じく、大きく跳んだら補間せず瞬間移動させること。**
- 歩行者は**擬似カメラにも映り、正解ラベルも付く**（`detections` の `cls: 5`）。
  車が当たれば `rewardCollision` の罰が入り、エピソードは衝突として終わる。
- 転送量は**64 人で 5,871B/frame ＝ 20Hz なら 114.7KB/s**（銀座・車両 4 台・検出込みの
  frame 全体は 31,915B）。減らしたいときは人数を下げること。
...
- **`braking` は加速度の実測ではなく指令**（`constrain_accel` を通したあとの値が負か）。
  エンジンブレーキや空気抵抗で減速しても実車の制動灯は点かないため。
- ★ **`throttle` は `braking` と同じ指令から出す**（`world.set_braking` の 1 か所）。
  クライアントはこれでアクセル・ブレーキペダルの踏み込みを描く。**別々の出どころに
  しないこと** — ブレーキランプが点いているのにブレーキペダルが戻っている、という
  食い違いが起きる。正がアクセル、負がブレーキで、`braking` は `throttle < -0.05`
  と一致する。走っていないスロットは 0。
- **`turnSignal` は経路の 30m 先の接線と現在の接線の差で決まる**（道交法施行令 21 条
  「右左折の 30m 手前」）。出す 0.35rad / 消す 0.12rad のヒステリシス付きで、
  緩いカーブで点いたり消えたりしない。**曲がっている最中も出たままになる**
  （消えるのは進路が揃ってから）。
- **前照灯とハザードはこのメッセージに入っていない。** 前照灯は天候（`frame.weather`）と
  昼夜から、ハザードは `taxi.phase` からクライアントが決める。**昼夜はサーバーが
  持っていない**（日の出・日の入りは `store/themeClock.ts` がクライアントで計算する）ため。
...
**段階が変わったとき・`vehicleId` が変わったとき・1Hz** で送られる。
...
- 乗降地点は**サーバー側で最寄りの道路ノード（交差点・端点）へ寄せられる**（決定 7）。
  クライアントが送った座標そのままではなく、`pickup` / `dropoff` に入って返る値が正。
  **区間の途中に置いてはいけない。** 経路はノード間で作るので、途中を目的地にすると
  「そこを通り過ぎて次の交差点まで行き、折り返して戻る」経路になり、
  停車は経路の終点で掛かるため**目的地の前を減速せずに通過する**
  （銀座で 64% の経路が平均 8.6m 行き過ぎていた）。
- 徴用した 1 台は**エピソードを閉じない**。到達しても respawn せず、乗降地点で停車する。
- ★ **`vehicleId` は配車の途中で変わることがある。** 迎えに向かっている車が来られなく
  なったとき（事故・車両の消失・道が塞がって近づけない）、サーバーは配車を畳まずに
  **別の車両へ引き継ぐ**。`phase` は `approaching` のままで `vehicleId` と `route` が
  差し替わり、`message` に「車両 #1 が来られなくなったため、車両 #5 が向かっています」が入る。
  **クライアントは `vehicleId` を毎回読み直すこと**（1 度きりだと古い車を追いかける）。
...
- ★ **`message` の中で車を指すときは `車両 #N` と書く**（半角スペース + `#` + 10 進）。
  クライアントはこの書式を見つけて**ナンバープレートの表記**（「品川 300 さ ・・・0」）へ
  置き換えて表示する（`frontend/src/scene/licensePlate.ts` の `withPlateNames()`）。
  地名はマップごとに変わり、その対応表はクライアントにしか無いのでここでは組み立てない。
  **書式を変えると置き換えが効かなくなり、番号のまま表示される**（壊れはしないが、
  画面の表記が揃わなくなる）。
...
あわせて `config.obsDim` が 57 から 66 へ変わっており、**それ以前に学習した重みは
読み込めない**（`rl/importer.py` が次元で弾く）。
