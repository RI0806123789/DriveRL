  ...
  "signals": [
    {
      ...
      "group": 0,              // 現示の番号。同じ交差点で同じ値どうしが同時に青になる
...
- 交差点 1 か所につき、**進入できる方向の数だけ**存在する。日本の信号機は
  ...
  一方通行の出口側には信号機を作らない
- `group` は進入方向の**軸**（向きの正負を無視した方向）でまとめた現示の番号で、
  **その交差点の現示の数だけ 0 から連番**になる。軸差が 30 度以上ある進入路は必ず
  別の番号に入るので、交差する流れが同時に青にならない。十字路は 2 つだが、
  **進入路が 3 本以上ある交差点は 3 つ以上になることがある**（実測: 金沢の 757 交差点のうち
  3 現示が 41・4 現示が 2）。交差点の現示の数は、その交差点の `group` の最大値 + 1
  （ただし最低 2）で、**サイクル 60 秒をその数で分け合う**（青 25 秒 → 3 現示なら 15 秒、
  4 現示なら 10 秒）
- **近接した灯器（8m 以内、または同じノード）は 1 つの交差点として扱い、位相も群も
  揃えてある**（揃えないと、同じ交差点の 2 基が片方青・片方赤になり、車はそこから
  動けず、歩行者はその灯器を裏返して渡ってくる）。`group` は**その交差点の中での
  区別**で、別の交差点の同じ値とは関係がない（交差点ごとに位相がずれている）
...
**現示の決まり方**: サイクルは 60 秒で、**交差点の現示の数で等分**する。
2 現示（十字路）なら青 25 秒 → 黄 3 秒 → 全赤 2 秒 →（次の現示へ交代）、
3 現示なら青 15 秒、4 現示なら青 10 秒。黄を挟まずに青から赤へ飛ぶことはなく、
...
- **`tick` は連番とは限らない。** 物理は `simHz × simSpeed` で進むが、フレームの生成は
  ...
  **クライアントは `tick` の差ではなく `simTime` の差で補間すること。** 受信した時刻で
  進めてもいけない（配信の間隔は Windows では 25〜94ms 揺れ、そのまま表示の揺れになる）。
  クライアントは `simTime` を時計にして、受信の揺れを吸収できるぶん遅らせて再生する（`store/playout.ts`）。
...
```jsonc
{
  ...
  "layers": [
    {
      ...
      "weightStd": 0.0688,
      "gradNorm": 0.0135,         // ★ 直近の更新で流れた勾配（**クリップ前**）。0 なら学習していない
  ...
  "logStdMin": -5.0,
  "logStdMax": 0.0,
  "gradTotalNorm": 0.82,          // ★ クリップ前の全体ノルム（ミニバッチ平均）
  "gradClipRate": 0.35,           // ★ クリップが効いたミニバッチの割合（0〜1）
  "gradMaxNorm": 0.5              // クリップの上限（`config.PPO_MAX_GRAD_NORM`）
}
```

`gradNorm` / `gradTotalNorm` は **`clip_grad_norm_` を掛ける前**に測る。
後で測ると全体ノルムが `gradMaxNorm` に丸められた値しか見えず、
**学習が止まったことは分かっても、勾配が暴れていることが分からない**。
暴れているかは `gradClipRate`（1.0 に張り付いていれば毎回クリップされている）で見る。
...
- ★ **`pickup` に入れるのは「利用者の現在地」である**（`store/pedestrian.ts` の座標）。
  画面で選ばせるのは降車地点だけで、乗車地点は選ばせない。サーバーから見た契約は
  変わらない（受け取った点を道路ノードへ寄せる）が、**クライアントは呼ぶ瞬間の
  現在地を入れること**。`pickup` は配車 1 件のあいだ動かない目標地点で、
  呼んだ後に利用者が歩いても追従しない。
- ★ **`phase` が `approaching` になっても、クライアントは利用者を乗車地点へ動かさないこと。**
  乗車地点は自分の足元なので、立たせ直すと**呼んだ瞬間に自分がワープする**。
...
### `player_pose` — 徒歩キャラの位置

実用モードで利用者が操作する徒歩キャラを、サーバーが**NPC 歩行者と同じ 1 人として**
街に置くためのメッセージ。これが無いと、車は目の前に立っても止まらず、轢いても
すり抜ける（サーバーは `frame.pedestrians` の NPC しか知らないため）。

- **送るのは 10Hz。** 位置はクライアント側で 60fps で動くが、サーバーは 20Hz で
  1 回しか読まない（`frame` を zustand に入れないのと同じ作法）。
- `at` が `null`、または値が読めないときは**街から消す**。10Hz で届くので、
  読めない値に `error` を返さない（同じ頻度で返し続けることになる）。
- ★ **乗車中（`phase` が `riding` / `arrived`）は送らないこと。** 送るとサーバーが
  乗客を歩行者として扱い、**自分を乗せた車が自分の前で止まって動けなくなる**。
  サーバー側も同じ条件で捨てるが、判断の出どころは 2 か所になるので両方で守る。
- **届かなくなってから 1 秒で自動的に消える**（`engine.PLAYER_POSE_TTL_SEC`）。
  タブを閉じた・回線が切れた利用者を街に残すと、そこに見えない人が立ち続けて
  車が永久に止まる。実用モードを抜けるときは `at: null` を 1 通送って即座に消す。
- 街に立つ人は**同時に 1 人だけ**。接続ごとには持たない（配車が同時に 1 件だけで、
  全接続へ同じ内容が配られるのと同じ前提）。複数のタブから送ると最後に届いた位置が勝つ。
- サーバーは位置を `frame` へ**返さない**。クライアントは自分のキャラを自分で描いており、
  返すと二重に描くことになる。
- ★ **`request_taxi` の `pickup` とは役割が違う。どちらか一方で済ませないこと。**
  `pickup` は配車 1 件のあいだ動かない**目標地点**（車が迎えに行く先）で、
  `player_pose` は届かなければ 1 秒で消える**いま立っている人**（車が止まる相手）。
